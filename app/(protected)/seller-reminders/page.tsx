...
import { SellerReminder } from '../../../types';
...
type RemindersResponse = { data: SellerReminder[]; total: number; page: number; limit: number };
...
export default function SellerRemindersPage() {
  ...
  const filtersRef = useRef({ search, statusFilter, startDate, endDate });

  useEffect(() => {
    filtersRef.current = { search, statusFilter, startDate, endDate };
  }, [search, statusFilter, startDate, endDate]);

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editing, setEditing] = useState<SellerReminder | null>(null);
  const [formState, setFormState] = useState({ title: '', content: '', remindAt: '' });
  const [saving, setSaving] = useState(false);

  const [toDelete, setToDelete] = useState<SellerReminder | null>(null);
  ...
  const fetchReminders = useCallback(async (targetPage = page, customFilters = filtersRef.current) => {
    ...
    try {
      ...
      const { data } = await api.get<RemindersResponse>('/seller-reminders', { params });
  ...
  }, [fetchReminders, search, statusFilter, startDate, endDate]);
  ...
  const openEdit = (r: SellerReminder) => {
    setEditing(r);
    setFormState({
      title: r.title ?? '',
      content: r.content ?? '',
  ...
  const toggleDone = async (r: SellerReminder) => {
    if (!isSeller) return;
    setError(null);
    try {
  ...
  if (!isSeller) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-8 text-sm text-gray-600">
        Somente vendedores podem acessar esta pagina.
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Registre lembretes com data e hora.</p>
        </div>
        <button
          onClick={openCreate}
